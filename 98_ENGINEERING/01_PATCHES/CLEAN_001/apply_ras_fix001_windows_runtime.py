...
def patch_runtime_scanner(path: Path) -> str:
    ...
    pattern = re.compile(
        ...
        r"(?P=indent)def\s+_scan_packages\(\s*self\s*\)",
        flags=re.MULTILINE,
    )
    ...
    if re.search(
        r"^[ \t]*def\s+_scan_packages\(\s*self\s*\)",
        text,
        flags=re.MULTILINE,
    ):
        return text
...
def main() -> int:
    print("=" * 72)
    print("RAS-FIX-001 Windows Path and Runtime Compatibility")
    print("=" * 72)
    ...
    try:
        for path in TARGETS:
            backup = path.with_name(
                f"{path.name}.bak_ras_fix001_{timestamp}"
            )
