...
"""
RAS-FIX-001 v3
Correct Windows path alias handling and RuntimeScanner binding.

This version inserts helper functions before the first decorator/class
block, so dataclass decorators remain attached to their classes.
"""
...
def insert_path_helper(text: str) -> str:
    ...
    # Insert before the first top-level decorator or class definition.
    match = re.search(
        r"^(?:@|class\s+)[A-Za-z_]",
        text,
        flags=re.MULTILINE,
...
def main() -> int:
    print("=" * 72)
    print("RAS-FIX-001 v3 Windows Path and Runtime Compatibility")
    print("=" * 72)
    ...
    try:
        for path in TARGETS:
            backup = path.with_name(
                f"{path.name}.bak_ras_fix001_v3_{timestamp}"
            )
